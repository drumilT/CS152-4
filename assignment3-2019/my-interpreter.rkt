...
(define (eval-program prog)
  (match prog
    [(pgm deflist) (foldl (lambda (x) (processdef x (top))) '() deflist) ]))

;;processdef describes how each definition is processed and added to
;;the frame fr.
(define (processdef defn fr)
  (match defn    
    [(def v/f exp) (hash-set! fr-bindings v/f (eval-exp exp))]
    )
  )
...
;[(lam? exp) (hash-set! fr-bindings v/f closure (exp) ( ) ]
;                        [(boolean? exp) (hash-set! fr-bindings v/f exp)]
;                       [(number? exp) (hash-set! fr-bindings v/f exp)]
;                      [(symbol? exp) (hash-set! fr-bindings v/f exp)]
;                     [(app? exp) (let [(n_frame (createframe (make-hash '()) fr))]
;                                  (begin (push n_frame)
(define (eval-exp exp)
  (cond [(symbol? exp) exp]
        [(boolean? exp) exp]
